  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full max-h-[90vh] overflow-auto">
        {/* Header */}
        <div className="sticky top-0 bg-white border-b-2 border-[#3B82F6] px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-[#0F172A]">{asset.title}</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-sm font-medium text-[#0F172A] mb-1">Status</p>
              <StatusBadge status={asset.status} />
            </div>

            <div>
              <p className="text-sm font-medium text-[#0F172A] mb-1">File Type</p>
              <p className="text-slate-700">{asset.content_type}</p>
            </div>

            {asset.subject && (
              <div>
                <p className="text-sm font-medium text-[#0F172A] mb-1">Subject</p>
            ...
            {asset.grade && (
              <div>
                <p className="text-sm font-medium text-[#0F172A] mb-1">Grade</p>
            ...
            {asset.file_size && (
              <div>
                <p className="text-sm font-medium text-[#0F172A] mb-1">File Size</p>
                <p className="text-slate-700">{(asset.file_size / 1024 / 1024).toFixed(2)} MB</p>
              </div>
            )}

            <div>
              <p className="text-sm font-medium text-[#0F172A] mb-1">Version</p>
              <p className="text-slate-700">v{asset.version}</p>
            </div>

            <div>
              <p className="text-sm font-medium text-[#0F172A] mb-1">Uploaded</p>
              <p className="text-slate-700 text-sm">{new Date(asset.created_at).toLocaleDateString()}</p>
            </div>

            {asset.created_by && (
              <div>
                <p className="text-sm font-medium text-[#0F172A] mb-1">Uploaded By</p>
          ...
          {asset.description && (
            <div>
              <p className="text-sm font-medium text-[#0F172A] mb-2">Description</p>
          ...
          {asset.extracted_text && (
            <div>
              <p className="text-sm font-medium text-[#0F172A] mb-2">Extracted Text Preview</p>
          ...
          {asset.metadata && Object.keys(asset.metadata).length > 0 && (
            <div>
              <p className="text-sm font-medium text-[#0F172A] mb-2">Additional Metadata</p>
        ...
        {/* Footer with Actions */}
        <div className="sticky bottom-0 bg-white border-t-2 border-[#3B82F6] px-6 py-4 flex gap-2 justify-end">
